...
This crew **does not use the Notion REST API**. Publishing uses **`notion_publish_srs_via_claude`** (Claude Code CLI `claude -p` + Notion MCP). A **final completeness pass** uses **`notion_verify_srs_publish_completeness_via_claude`** (same stack) to diff SRS vs live pages and patch gaps. You need a **root** parent: `NOTION_COPILOT_SRS_PARENT_PAGE_ID` (UUID) and/or `NOTION_COPILOT_SRS_PARENT_URL` (fallback: `NOTION_SRS_PARENT_*`), plus the SRS file on disk (see `read_srs_for_notion_publish` / `ARVO_COPILOT_SRS_PUBLISH_INPUT`). Artefacts are written under `outputs/copilot/notion_export/`. No `NOTION_API_KEY` is required for this flow.
